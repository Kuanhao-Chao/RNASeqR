...
RawReadsBoxPlot <- function(path.prefix) {
  ...
  png(paste0(path.prefix, "RNAseq_results/Raw_reads/gene/Box_plot.png"),  width = 800, height = 800)
  ...
  p1 <- p1 + theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title=element_text(size=14), plot.title = element_text(size = 40, face = "bold", hjust = 0.5))
  print(p1)
  dev.off()
  png(paste0(path.prefix, "RNAseq_results/Raw_reads/transcript/Box_plot.png"),  width = 600, height = 600)
  ...
  p2 <- p2 + theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title=element_text(size=14), plot.title = element_text(size = 30, face = "bold", hjust = 0.5))
  print(p2)
  dev.off()
}
...
RawReadsViolinPlot <- function(path.prefix) {
  ...
  colnames(melt.data.gene) <- c("samples", "raw_reads_count")
  colnames(melt.data.transcript) <- c("samples", "raw_reads_count")
  png(paste0(path.prefix, "RNAseq_results/results/raw_reads/gene/Violin_plot.png"),  width = 600, height = 600)
  ...
  print(p1)
  dev.off()
  png(paste0(path.prefix, "RNAseq_results/results/raw_reads/transcript/Violin_plot.png"),  width = 600, height = 600)
