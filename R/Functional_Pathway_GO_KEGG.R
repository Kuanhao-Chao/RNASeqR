...
#       gse <- clusterProfiler::gseGO(geneList     = gene_list_ENTREZID,
#                                     keyType      = "ENTREZID",
#                                     OrgDb        = OrgDb.species,
#                                     ont          = i,
#                                     pvalueCutoff = 0.05)
...
#         message(paste0("          \u25CF (\u2714) GO Gene Set Enrichment test (", i,") enriched term found! \n"))
#         message(paste0("               \u25CF Writing 'GO_", i, "_Gene_Set_Enrichment.csv' \n"))
...
#         message(paste0("          \u25CF Checking 'GO_", i, "_Gene_Set_Enrichment.csv' result row number \n"))
#         if (length(row.names(gse.data.frame)) < 5) {
#           message(paste0("               \u25CF 'GO_", i, "_Gene_Set_Enrichment.csv' result row number : ", length(row.names(gse.data.frame)), " (less than 5)\n"))
#           iterate.term <- gse.data.frame$ID
#         } else {
#           message(paste0("               \u25CF 'GO_", i, "_Gene_Set_Enrichment.csv' result row number : ", length(row.names(gse.data.frame)), " (more than 5)\n"))
#           iterate.term <- gse.data.frame$ID[seq_len(5)]
#         }
#         for(GO.ID in iterate.term) {
#           message(paste0("                    \u25CF GO ID : ", GO.ID, "\n"))
...
#           message(paste0("                         \u25CF Plotting '", GO.ID, "_gseGO_Plot_clusterProfiler.png'\n"))
...
#     #   message(paste0("               \u25CF Plotting 'GO_", i, "_Enrichment_Map_plot.png' \n"))
...
#     #   message(paste0("               \u25CF Plotting 'GO_", i, "_Enrichment_Complex_plot.png' \n"))
...
#     #   message(paste0("               \u25CF Plotting 'GO_", i, "_Enrichment_Induced_plot.png' \n\n"))
#     #   p6 <- clusterProfiler::goplot(ego)
#     #   print(p6)
#     #   dev.off()
#     # } else {
#     #   message(paste0("               \u25CF Row size of 'GO_", i,"_Overrepresentation.csv' is smaller than 2. Can't draw.\n\n"))
...
#   }
#   DE_results <- DEGeneList(which.analysis, path.prefix, OrgDb.species, input.TYPE.ID)
#   gene_list_input_TYP = DEUniv_results$gene_list_input_TYP_rt
...
#   # DO KEGG Gene Set Enrichment Analysis
#   kk.gse <- clusterProfiler::gseKEGG(geneList     = gene_list_ENTREZID,
#                                      organism     = "sce")
#
