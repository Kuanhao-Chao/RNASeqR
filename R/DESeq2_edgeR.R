edgeRRawCountAnalysis <- function(path.prefix, independent.variable, control.group, experiment.group) {
  ...
  cat("\n\u25CF Plotting edgeR MeanVar plot ... ")
  png(paste0(path.prefix, "RNAseq_results/Reads_Count_Matrix_analysis/edgeR/images/MeanVar_plot.png"))
  p <- edgeR::plotMeanVar(dgList, show.tagwise.vars=TRUE, NBline=TRUE)
  print(p)
  dev.off()

  cat("\n\u25CF Plotting edgeR BCV plot ... ")
...
DESeq2RawCountAnalysis <- function(path.prefix, independent.variable,  control.group, experiment.group, DESeq2.padj, DESeq2.log2FC) {
  ...
  cat(paste0("\u25CF Writing '", path.prefix, "RNAseq_results/Reads_Count_Matrix_analysis/DESeq2_", control.group, "_vs_", experiment.group, "'"))
  write.csv(res, file = paste0(path.prefix, "RNAseq_results/Reads_Count_Matrix_analysis/DESeq2_", control.group, "_vs_", experiment.group))
  cat(paste0("\n\u25CF Plotting DESeq2 MA plot"))
  ...
  sig <-res.sort.padj[(!is.na(resOrdered$padj)) && (resOrdered$padj < DESeq2.padj) && (abs(resOrdered$log2FoldChange) >= DESeq2.log2FC)]

  # plot plotDispEsts
  # plotDispEsts
  cat(paste0("\n\u25CF Plotting DESeq2 Dispersion plot"))
