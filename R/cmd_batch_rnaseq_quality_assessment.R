...
RNASeqQualityAssessment_CMD <- function(RNASeqWorkFlowParam, run = TRUE, check.s4.print = TRUE) {
  ...
  close(fileConn)
  message(paste0("\u2605 '", path.prefix, "Rscript/Quality_Assessment.R' has been created.\n"))
  if (run) {
    ...
    message(paste0("\u2605 Tools are installing in the background. Check current progress in '", path.prefix, "Rscript_out/Quality_Assessment.Rout'\n\n"))
  }
}
...
RNASeqQualityAssessment <- function(path.prefix, input.path.prefix, sample.pattern) {
  ...
  # message(paste0("     \u25CF  Running 'rqcQA()' ...  Please wait \u231B\u231B\u231B\n"))
  # qa <- Rqc::rqcQA(files)
  # message(paste0("     \u25CF  Creating 'rqc_report.html' ...  Please wait \u231B\u231B\u231B\n"))
  ...
  message(paste0("     \u25CF  Running 'seeFastq()' ...  Please wait \u231B\u231B\u231B\n"))
  fqlist <- systemPipeR::seeFastq(fastq=systemPipeR::infile1(args), batchsize=10000, klength=8)
  message(paste0("     \u25CF  Creating 'fastqReport.pdf' ...  Please wait \u231B\u231B\u231B\n"))
  ...
  message(paste0("     \u25CF  Removing 'RNASeq' directory...  Please wait \u231B\u231B\u231B\n"))
  ...
  # files <- list.files(folder, sample.pattern, full.names=TRUE)
  # message(paste0("     \u25CF  Running 'qa()' ...  Please wait \u231B\u231B\u231B\n"))
  # qaSummary <- ShortRead::qa(files, type="fastq")
  # message(paste0("     \u25CF  Creating 'ShortRead_report.html' ...  Please wait \u231B\u231B\u231B\n"))
...
PreCheckRNASeqQualityAssessment <- function(path.prefix, sample.pattern) {
  message("\u269C\u265C\u265C\u265C 'RNASeqQualityAssessment()' environment pre-check ...\n")
...
PostCheckRNASeqQualityAssessment <- function(path.prefix) {
  message("\u269C\u265C\u265C\u265C 'RNASeqQualityAssessment()' environment post-check ...\n")
  ...
  if (!isTRUE(validity)) {
    # if (!file.rqc.result) {
    #   message(paste0("'", path.prefix, "RNASeq_results/QA_results/QA_", QA.count, "/Rqc/Rqc_report.html' is missing!\n"))
    # }
    if (!file.systemPipeR.data) {
      message(paste0("'", path.prefix, "RNASeq_results/QA_results/QA_", QA.count, "/systemPipeR/data.list.txt' is missing!\n"))
    }
    if (!file.systemPipeR.result){
      message(paste0("'", path.prefix, "RNASeq_results/QA_results/QA_", QA.count, "/systemPipeR/fastqReport.pdf' is missing!\n"))
    }
    # if (!file.ShortRead.result) {
    #   message(paste0("'", path.prefix, "RNASeq_results/QA_results/QA_", QA.count, "/ShortRead/ShortRead_report.html' is missing!\n"))
    # }
