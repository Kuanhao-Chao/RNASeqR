...
RNASeqBallgownDESeq2EdgeRProcess_CMD <- function(RNASeqWorkFlowParam, ballgown.qval = 0.05, ballgown.log2FC = 1, TPM.pval = 0.05, TPM.log2FC = 1, DESeq2.padj = 0.1, DESeq2.log2FC = 1, edgeR.pval = 0.05, edgeR.log2FC = 1, run = TRUE, check.s4.print = TRUE) {
  # check input param
...
#'                            experiment.type = "two.group", main.variable = "treatment", additional.variable = "cell")
#' RNASeqEnvironmentSet_CMD(RNASeqWorkFlowParam = exp)}
RNASeqBallgownDESeq2EdgeRProcess <- function(path.prefix, genome.name, sample.pattern, independent.variable, control.group, experiment.group, ballgown.qval = 0.05, ballgown.log2FC = 1, TPM.pval = 0.05, TPM.log2FC = 1, DESeq2.padj = 0.1, DESeq2.log2FC = 1, edgeR.pval = 0.05, edgeR.log2FC = 1) {
  CheckOperatingSystem(FALSE)
