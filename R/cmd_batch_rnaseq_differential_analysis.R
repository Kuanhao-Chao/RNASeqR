...
#'
#' @description This function will run differential analysis on ballgown, DESeq2 and edgeR.
#'   TPM values will be converted from ballgown FPKM values, and T-test will be made to calculate pvalue.
...
#'      Independent sample t-test is used with TPM values as input. P-values will be produced after t-test.
#'       Fold change values are calculated by the formula : "average TPM experiment group values divide
#'       average TPM control group values".
#'   3. DESeq2 analysis
#'      Median of rations normalization(MRN) is used in DESeq2 for raw read count normalization.
...
#'   4. edgeR analysis
#'      Raw reads are normalized by TMM and library size. (run \code{calcNormFactors()} to get a DGEList,
#'      and then run \code{cpm()} on that DGEList)
#'      The main statistic test in edgeR is trimmed mean of M-values(TMM).
#'   If you want to run differential analysis on ballgown, TPM normalization, DESeq2, edgeR
#'   for the following RNA-Seq workflow in R shell, please see \code{RNASeqDifferentialAnalysis()} function.
#'
#' @param RNASeqWorkFlowParam S4 object instance of experiment-related parameters
#' @param ballgown.pval Default \code{0.05}. Set the threshold of ballgown p-value to
#'   filter out differential expressed gene.
#' @param ballgown.log2FC Default \code{1}. Set the threshold of ballgown log2 fold change to
#'   filter out differential expressed gene.
#' @param TPM.pval Default \code{0.05}. Set the threshold of TPM & student t-test p-value to
#'   filter out differential expressed gene.
#' @param TPM.log2FC Default \code{1}. Set the threshold of TPM & student t-test log2 fold change to
#'   filter out differential expressed gene.
#' @param DESeq2.pval Default \code{0.05}. Set the threshold of DESeq2 p-value to filter out
#'   differential expressed gene.
#' @param DESeq2.log2FC Default \code{1}. Set the threshold of DESeq2 log2 fold change to
#'   filter out differential expressed gene.
#' @param edgeR.pval Default \code{0.05}. Set the threshold of edgeR p-value to filter
#'   out differential expressed gene.
#' @param edgeR.log2FC Default \code{1}. Set the threshold of edgeR log2 fold change to
#'   filter out differential expressed gene.
...
#' @description This function will run differential analysis on ballgown, DESeq2 and edgeR.
#'   TPM values will be converted from ballgown FPKM values, and T-test will be made to calculate pvalue.
#'   It is strongly advised to run \code{RNASeqDifferentialAnalysis_CMD()} directly.
#'   Running \code{RNASeqDifferentialAnalysis_CMD()} will create 'Differential_Analysis.Rout' file in 'Rscript_out/' directory.
...
#'      TPM normalization is calculated from the FPKM values produced by ballgown.
#'      Independent sample t-test is used with TPM values as input. P-values will be produced after t-test.
#'      Fold change values are calculated by the formula : "average TPM control group values divide average
#'      TPM case group values".
#'   3. DESeq2 analysis
#'      Median of rations normalization(MRN) is used in DESeq2 for raw read count normalization.
#'      Sequencing depth and RNA composition is taken into consideration is this normalization method.
#'      The main statistic test in DESeq2 is negative binomial distribution.
...
#'      and then run \code{cpm()} on that DGEList)
#'      The main statistic test in edgeR is trimmed mean of M-values(TMM).
#'   If you want to run differential analysis on ballgown, TPM normalization, DESeq2, edgeR for the following
#'   RNA-Seq workflow in background, please see \code{RNASeqDifferentialAnalysis()} function.
#'
...
#' @param TPM.log2FC Default \code{1}. Set the threshold of TPM & student t-test log2 fold change to
#'   filter out differential expressed gene.
#' @param DESeq2.pval Default \code{0.05}. Set the threshold of DESeq2 p-value to
#'   filter out differential expressed gene.
#' @param DESeq2.log2FC Default \code{1}. Set the threshold of DESeq2 log2 fold change to
#'   filter out differential expressed gene.
#' @param edgeR.pval Default \code{0.05}. Set the threshold of edgeR p-value to
#'   filter out differential expressed gene.
#' @param edgeR.log2FC Default \code{1}. Set the threshold of edgeR log2 fold change to
#'   filter out differential expressed gene.
#'
