...
CheckInputParamNa <- function(path.prefix, input.path.prefix, genome.name,
  ...
  if (is.na(path.prefix) || is.na(input.path.prefix) ||
    ...
    message(paste0("(\u2714) : Input parameters are all not NA !! \n\n"))
  }
}
...
CheckOperatingSystem <- function(print = TRUE){
  ...
  os <- tolower(os)
  if (os != "linux" && os != "osx") {
    message(paste0("(\u2718) : Your system operating system is ", os, "\n"))
...
CheckInputDirFiles <- function(input.path.prefix, genome.name, sample.pattern) {
  ...
  if (isTRUE(raw.fastq.dir)) {
    ...
    if (sample.pattern == "") {
      message(paste0("(\u2718) : The value of 'sample.pattern' is \"\" !!\n"))
  ...
  if (!isTRUE(phenodata.file)) {
    message(paste0("(\u2718) : '", "phenodata.csv is missing.\n"))
  } else {
    message(paste0("(\u2714) : '", "phenodata.csv is in 'input_files'\n"))
  }
  if (!isTRUE(ht2.dir)) {
    ## not exist
