...
6. **Reads alignment & Transcript assembly**: 
  ...
  assembled into potential transcripts by assembler. Another different approach 
...
The central concept behind this package is that **each step involved in RNA-Seq data analysis is a function call in R**. For the subsequent parts of this documentation, inputs, outputs as well as detail implementation for these six steps will be elaborated by order. Following are six steps and each corresponding function that users need to execute.

1. **`RNASeqRParam` S4 Object Creation**
    * **Function: `RNASeqRParam()`** 
    * In the beginning, users will create a `RNASeqRParam` S4 object by running the `RNASeqRParam()` constructor function for all variable checking. After the creation of `RNASeqRParam` S4 object, it will be used as input to the following analysis functions.

2. **Environment Setup**
    * **Function: `RNASeqEnvironmentSet_CMD()` or `RNASeqEnvironmentSet()`**
    * In second step, the environemnt would be set up automatically with necessary tools installed for the subsequent steps in RNASeqR.

3. **Quality Assessment (optional)**
    * **Function: `RNASeqQualityAssessment_CMD()` or `RNASeqQualityAssessment()`** 
    * In third step (optional), users check the quality of raw fastq files.

4. **Reads alignment and transcripts quantification**
    * **Function: `RNASeqReadProcess_CMD()` or `RNASeqReadProcess()`**
    * In fourth step, raw reads would be processed and all intermediate files would be stored properly.

5. **Differential Expression Gene Analysis**
    * **Function: `RNASeqDifferentialAnalysis_CMD()` or `RNASeqDifferentialAnalysis()`**
    * In fifth step, differential analysis would be done by edgeR, DESeq2 and ballgown R / Bioconductor packages.

6. **Functional Analysis**
    * **Function: `RNASeqGoKegg_CMD()` or `RNASeqGoKegg()`**
    * In sixth step, differential expressed genes would be used as input of GO functional analysis and KEGG pathway analysis. Enriched pathways would be visualized.
...
Check the reads files in *'input_files/'* directory. Users could set *fastq.gz.type* as "PE" (paired-end) or "SE" (single-end) to specify which kinds of fastq.gz types they are.
...
The main analysis workflow of RNASeqR starts from  raw fastq.gz files. However, we also allow users to run the pipeline from `SAM` files or `BAM` files. Similar to the first step mentioned in section **"RNASeqRParam S4 Object Creation"**, users need to create a different S4 Object: `RNASeqRParam_Sam` for starting from `SAM` files or `RNASeqRParam_Bam` starting from `BAM` files.
...
1. **`RNASeqRParam_Sam` or `RNASeqRParam_Bam` S4 Object Creation**
    ...
    * In the beginning, users will create a `RNASeqRParam_Sam` or `RNASeqRParam_Bam` S4 object by running the `RNASeqRParam_Sam()` or `RNASeqRParam_Bam()` constructor function for all variable checking. After the creation of `RNASeqRParam_Sam()` or `RNASeqRParam_Bam()` S4 object, it will be used as input to the following analysis functions.
