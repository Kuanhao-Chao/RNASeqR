...
Here we illustrate general data visualization before and after differential expression analysis. The results based on each differential analysis tool(*ballgown*, *t-test on TPM*, *DESeq2*, *edgeR*) are kept in the directory 'RNASeq_results/' separately. The statistical results based on MRN-normalized value through DESeq2 R package are presented as follows: 
...
* **'Heatmap_Plot_pheatmap.png'**: To visualize the expression value of DEGs in terms of log2-based normalized value from all samples of two groups using [pheatmap](https://cran.r-project.org/web/packages/pheatmap/index.html) package. Samples are colored by defined groups: blue for case group and yellow for control 2.
