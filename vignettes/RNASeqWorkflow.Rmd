...
This is the **pre-step** of RNA-Seq workflow in this package. Before starting RNA-Seq analysis, you have to run the constructor function `RNASeqWorkFlowParam()` and create `RNASeqWorkFlowParam` S4 object first. This object will be used as input parameter in the following analysis function.
...
1. Create a new directory for RNA-Seq analysis.
    * It is highly recommended to create a new directory without any files inside. The parameter `path.prefix` of `RNASeqWorkFlowParam()` constructor should be the absolute path of this new directory. All the RNA-Seq related files that generated in the following steps will be stored inside of this directory.

2. Create valid *'input_files/'* directory
    * You should create a file directory named *'input_files/'* with neccessary files inside. It should follow the rules metioned above.
...
### Files Setup
1. Create Base Directories *'gene_data/'*, *'RNASeq_bin/'*, *'RNASeq_results'*, *'Rscript'* and *'Rscript_out'* will be created under `path.prefix` directory.

2. Copy *'input_files/'* symbolic links will be created from files in *'input_files/'* to `path.prefix` directory. 
...
This is the **third necessary step** of RNA-Seq workflow in this package. To do differential analysis(DA), run `RNASeqDifferentialAnalysis_CMD()` to execute process in background or run `RNASeqDifferentialAnalysis()` to execute process in R shell. In `RNASeqWorkflow` package, we provide four ways to do differential analysis.

The reads sum of each row equals `0` in *gene_data/reads_count_matrix/gene_count_matrix.csv*, which is generated in the previous step, will be filtered out.
