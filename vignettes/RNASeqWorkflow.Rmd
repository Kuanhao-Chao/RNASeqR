---
title: "RNASeqWorkflow : RNA-Seq differential expression analysis based on one independent variable analysis(??????)"
...
In this step, [ShortRead](http://bioconductor.org/packages/release/bioc/html/ShortRead.html) package is ultilized for trimming the bad quality ends off the reads and the details are as follows:
...
1. Run in background
Result will be reported in *'Rscript_out/Quality_Trimming.Rout'*. Make sure quality trimming is successfully finished before running the subsequent steps.
...
2. Run in R shell 
Result will be reported in R shell. Make sure quality assessment is successfully finished before running the subsequent steps.
...
This is the **third necessary step** of RNA-Seq workflow in this package. To identify differential expressed genes, users can either run `RNASeqDifferentialAnalysis_CMD()` to execute process in background or run `RNASeqDifferentialAnalysis()` to execute process in R shell. In this package, we provide four normalized expression values, Fragments Per Kilobase per Million(FPKM)(Mortazavi et al. 2008), Transcripts Per Kilobase Million(TPM)(Li et al. 2009), normalized counts by means of Median of Ratios Normalization(MRN) or Trimmed Mean of M-values(TMM), with proper statistical analyses using R packages, 'ballgown', 'stats', 'DESeq2' and 'edgeR'. Gene IDs from StringTie and Ballgown R package will be mapped to 'gene_name' in GTF file for further functional analysis.

## Tabular Format for Reads Alignment

As long as users finished running `RNASeqRawReadProcess_CMD()` ahead, summary reports for alignment rates in terms of tabular(`CSV`) and picture(`PNG`) format are created and kept at the directory 'Rscript_out/Read_Process.Rout'.
...
Here we illustrate general data visualization before and after differential expression analysis. The results based on each type of normalized value are kept in the directory, 'RNASeq_results/'. The statistical result based on normalized counts by means of median of ratios normalization(MRN) through function plots(DESeq2) are presented as follows: (??????????)
...
```{r fig.width=8, height=8, echo=FALSE}
...
To display the pearson correlation coefficient of a pairwise correlation analysis of changes in gene expression from all samples calculated by [stats](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/00Index.html) package using [ggplot2](https://cran.r-project.org/web/packages/ggplot2/index.html), [corrplot](https://cran.r-project.org/web/packages/corrplot/index.html) and [PerformanceAnalytics](https://cran.r-project.org/web/packages/PerformanceAnalytics/index.html) packages. The colors from red to blue mark the value of the coefficient from maximum value to minimum value among all samples.
...
To display how the biological samples compare in similarities and difference based on the expression value of DEGs using principal component analysis. [FactoMineR](https://cran.r-project.org/web/packages/FactoMineR/index.html) and  [factoextra](https://cran.r-project.org/web/packages/factoextra/index.html) are used in this step.
...
To visualize the expression value of DEGs in terms of log2-based normalized value from all samples of two groups using [pheatmap](https://cran.r-project.org/web/packages/pheatmap/index.html) package. Samples are colored by defined groups: blue for group 1 and yellow for group 2.
...
7. Additional Data Visualization. Aside from general data visualization mentioned above, MA plot is also provided.(?????????????)
...
* **'MA_Plot_ggplot2.png'**: To display the difference of expression value between two groups by transforming the data onto log2-based ratio (x-axis) and log2-based mean (y-axis) scales.
```{r fig.width=8, height=8, echo=FALSE}
img <- readPNG("figure/ballgown_MA_Plot_ggplot2.png")
grid.raster(img, just = "center")
```

## Student t-test Analysis Based on TPM Value

Similar to FPKM value, TPM(Transcripts Per Kilobase Million) value is derived from reads count normalized by library size and transcript length using a slightly different approach, resulting in providing better cross-sample comarability. The statistic model used here is Student’s t-test. The basic steps are as follows:

* Input: *'gene_data/ballgown/'*
* Output: *'RNASeq_results/TPM_analysis/'*

1. Convert TPM values from FPKM values extracted from StringTie in column `sample.pattern.FPKM`.

2. Apply Student’s t-test to estimate  p-value in column `pval`.
...
4. Generate a `CSV` file, *'RNASeq_results/TPM_analysis/TPM_normalized_result.csv'*, to store normalized TPM values, mean expression values per group and statistic results

5. Select DEGs based on default criteria: **`pval < 0.05` and `log2FC > 1 | log2FC < 1`**, and store the result in *'RNASeq_results/TPM_analysis/TPM_normalized_DE_result.csv'*.

6. Additional Data Visualization. Aside from general data visualization mentioned above, MA plot is also provided.

* **MA Plot**: To display the differences of expression value between two groups by transforming the data onto log2-based ratio (x-axis) and log2-based mean (y-axis) scales.
```{r fig.width=8, height=8, echo=FALSE}
...
* **Dispersion Plot**: To display the dispersion estimates before and after normalization using `plotDispEsts()`. The x-axis denotes the mean of normalized counts while y-axis represents the dispersion estimates value.
```{r fig.width=8, height=8, echo=FALSE}
img <- readPNG("figure/DESeq2_Dispersion_Plot_DESeq2.png")
grid.raster(img, just = "center")
```

* **MA Plot**: To display the diﬀerences of expression value between two groups by transforming the data onto log2-based ratio (x-axis) and log2-based mean (y-axis) scales.
```{r fig.width=8, height=8, echo=FALSE}
...
7. Additional Data Visualization. Aside from general data visualization mentioned above, MeanVar plot, BCV and MDS plot are also provided.

* **MeanVar Plot**: To visualize the mean-variance relationship before and after TMM normalization using `plotMeanVar()` function.
```{r fig.width=8, height=8, echo=FALSE}
img <- readPNG("figure/edgeR_MeanVar_Plot_edgeR.png")
grid.raster(img, just = "center")
```

* **BCV Plot**: To display the genewise biological coefficient of variance(BCV) against gene abundance using `plotBCV()` function.
```{r fig.width=8, height=8, echo=FALSE}
img <- readPNG("figure/edgeR_BCV_Plot_edgeR.png")
grid.raster(img, just = "center")
```

* **MDS Plot**: To present expression differences between the samples using `plotMDS.DGEList()` function.
```{r fig.width=8, height=8, echo=FALSE}
img <- readPNG("figure/edgeR_MDS_Plot_edgeR.png")
grid.raster(img, just = "center")
```

* **Smear Plot**: To plot log2-based fold change against the log10-based concentration.
```{r fig.width=8, height=8, echo=FALSE}
